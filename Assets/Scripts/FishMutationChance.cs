...
public class FishMutationChance : MonoBehaviour
{
    ...
    public void CheckMutationOnBought(ref float maxHunger, ref float maxSize, ref float maxPrice, ref float movementSpeed, ref Material color)
    {
        if (IsStatMutate(chanceOnBought))
        {
            ...
            color = materials[Random.Range(0, materials.Length)];
        }
    }

    public void CheckMutationOnBreed(ref float maxHunger, ref float maxSize, ref float maxPrice, ref float movementSpeed, ref Material color, FishStats momFish, FishStats dadFish)
    {
        ...
        color = materials[materials.Length - 1];
    }
}
